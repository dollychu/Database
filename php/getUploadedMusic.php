    ...
    <a style="text-decoration:none;" href="musicPlayer.php?path={$song['Path']}">
      <div class="card-body text-dark">
        ...
        <div class="row mb-2"> 
          ...
          <div class="col-9"><p class="card-text"> {$song['Description']} </p></div>
        </div>
...
_END;
$button = "<button type='button' class='btn btn-outline-danger mb-2 mx-auto' id='{$song['IdMusic']}_{$_SESSION['IdUser']}'>
             Remove
           </button>";
...
function getUpload($uid){
  ...
  if($result = $conn->query($query)){
    ...
    while($row = $result->fetch_array()){
      $assoc = array('UserName'=>$row[0], 'MusicName'=>explode('.', $row[1])[0], 'Path'=>$row[2], 'Description'=>$row[3], 
                     'UploadDate'=>pathinfo($row[4], PATHINFO_FILENAME), 'IdMusic'=>$row[5], 'UploadUser'=>$row[6]);
      $return_v[] = $assoc;
    }
